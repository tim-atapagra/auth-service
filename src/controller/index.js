...
exports.login = async (req, res) => {
  ...
  if (!user)
    return res
      .status(404)
      .send("The username does not exist, please sign up for a new account");

  const authUser = await userService.findOne({ userId: user.userId });

  if (!authUser.accountVerified)
    return res
      .status(404)
      .send("Please check your phone to verify account before proceeding");
  ...
  if (passCheck) {
    ...
    return res
      .status(404)
      .send("Username or Password does not match, Please check again");
  }
};
...
exports.signup = async (req, res) => {
  ...
  const [hash, token, sanitizedEmail] = await Promise.all([
...
exports.validate = async (req, res) => {
  ...
  return res
    .status(200)
    .send("Your account is successfully confirmed! Proceed to login");
};
